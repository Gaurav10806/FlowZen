"""
Enhanced Conditional (IF/ELSE) Node

This module provides a simple, safe conditional node for workflow decision-making.
Supports basic comparison operations without arbitrary code execution.
"""
...
@register_node
class ConditionalNode(UtilityNode):
    """
    Enhanced Conditional Node - Simple IF/ELSE logic for workflows.
    
    Provides safe, deterministic conditional branching based on data comparison.
    Supports multiple operators and operand types without code execution risks.
    ...
    DISPLAY_NAME = "IF / ELSE Condition"
    DESCRIPTION = "Branch workflow execution based on simple IF/ELSE conditions"
    CATEGORY = "utilities"
    SUPPORTS_RETRY = True
    ...
    def run(self, input_data: Dict[str, Any], params: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        Evaluate conditional logic and determine execution path.
        
        Args:
            input_data: Data from previous nodes
            params: Conditional configuration
                - left_operand: Left side of comparison (field path or literal)
                - operator: Comparison operator
                - right_operand: Right side of comparison (field path or literal)
                - case_sensitive: Whether string comparisons are case sensitive
            context: Execution context
            
        Returns:
            Dict with condition result and execution path information
        """
        self.logger.info("Evaluating conditional logic")
        ...
        if operator not in self.OPERATORS:
            raise NodeExecutionError(
                f"Invalid operator '{operator}'. Supported: {list(self.OPERATORS.keys())}"
            )
        ...
        try:
            ...
            self.logger.debug(f"Comparing: {left_value} {operator} {right_value}")
            ...
            self.logger.info(f"Condition result: {condition_result} -> path: {execution_path}")
            
            # Return result with execution path information
            return {
    ...
    def _resolve_operand(self, operand: Union[str, Any], input_data: Dict[str, Any], context: Dict[str, Any]) -> Any:
        ...
        Operands can be:
        1. Field paths (e.g., "user.email", "order.total")
        2. Template variables (e.g., "{{user.name}}")
        3. Literal values (strings, numbers, booleans)
        4. JSON strings that should be parsed
    ...
    def get_schema(cls) -> Dict[str, Any]:
        """
        Return JSON schema for conditional node parameters.
        """
        return {
            "type": "object",
            "properties": {
                "left_operand": {
                    "type": "string",
                    "title": "Left Operand",
                    "description": "Left side of comparison (field path, template, or literal value)",
                    "examples": ["user.email", "{{order.total}}", "100", "active"]
                },
                "operator": {
                    "type": "string",
                ...
                "right_operand": {
                    "type": "string", 
                    "title": "Right Operand",
                    "description": "Right side of comparison (field path, template, or literal value)",
                    "examples": ["admin@example.com", "{{config.threshold}}", "0", "true"]
                },
                "case_sensitive": {
                    "type": "boolean",
        ...
        return {
            ...
            "properties": {
                "condition_result": {
                    "type": "object",
                    "properties": {
                        "result": {"type": "boolean", "description": "True/False result of condition"},
                        "execution_path": {"type": "string", "enum": ["true", "false"]},
            ...
            "additionalProperties": True,
            "description": "All input data plus condition_result with evaluation details"
        }
