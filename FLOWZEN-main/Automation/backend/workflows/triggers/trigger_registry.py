...
class TriggerRegistry:
    ...
    def get_available_trigger_types(self) -> List[Dict[str, Any]]:
        ...
        for trigger_type in TriggerType:
            try:
                ...
                elif trigger_type == TriggerType.WHATSAPP:
                    trigger_info.update({
                        'real_time': True,
                ...
                elif trigger_type == TriggerType.CHAT:
                    trigger_info.update({
                        'real_time': True,
                        'supports_custom_response': True
                    })
                elif trigger_type == TriggerType.EMAIL:
                    trigger_info.update({
                        'real_time': False,
                        'supports_authentication': True
                    })
    ...
    def validate_trigger_configuration(self, workflow, trigger_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif trigger_type == TriggerType.WHATSAPP.value:
            # WhatsApp Trigger mainly needs a credential, which is validated by the node schema validation usually.
            # But we can add extra checks here if needed.
            if not params.get('credential_id'):
                errors.append('WhatsApp trigger requires credential_id')
    ...
    def get_trigger_status(self, workflow) -> Dict[str, Any]:
        ...
        if trigger_type == TriggerType.WEBHOOK.value:
            ...
            status.update({'active': True, 'message': 'Managed via Meta Webhook'})
        else:
    ...
    def activate_trigger(self, workflow, trigger_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            elif trigger_type in [TriggerType.TELEGRAM.value, TriggerType.WHATSAPP.value, TriggerType.CHAT.value]:
                # These triggers are handled globally via webhooks/sockets
                return {'success': True, 'message': f'{trigger_type} is active globally'}
            else:
                return {
                    'success': False,
    ...
    def deactivate_trigger(self, workflow, trigger_type: str) -> Dict[str, Any]:
        ...
        try:
            if trigger_type == TriggerType.WEBHOOK.value:
                ...
                return {'success': True, 'message': 'WhatsApp trigger deactivated'}
            else:
                return {
                    'success': False,
                    'error': f'Unknown trigger type: {trigger_type}'
